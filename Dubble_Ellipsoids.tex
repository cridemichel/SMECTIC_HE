...
\end{figure*}
In former studies~\cite{martchenko2016anisotropic} hematite-silica core-shell particles 
were simply modeled by HEs, anyway our closer inspection to their shape reveals that the scenario is 
more diverse. Hematite-silica core-shell particles of aspect ratio $\rho=3.69$  (see~\ref{z_scan}(b) and also the SI)
closely resemble hard ellipsoids (HEs). In contrast, particles with smaller aspect ratio $\rho=2.82$ possess an  
hybrid shape, where their midsection is more cylinder-like (i.e.~flatter), as the one of hard spherocylinders (HSCs), 
while their ends resemble more those of uni-axial hard ellipsoids (HEs) as shown in Fig.\ref{z_scan}(a).
%In contrast, particles with larger aspect ratio $\rho_2$ are overall more similar to HEs.
Hence
%, to model hematite-silica particles with $\rho_1=2.82$, 
we decided to study both HEs and HSCs, as shown in Figs.~\ref{fig:sim}a and~\ref{fig:sim}b respectively, 
...
We observe that hematite-silica core-shell particles are charged but the resulting repulsive interaction between them 
can be accounted for by just considering a larger particle volume. Hence, we expect that electrostatic repulsion between particles 
induces a shift of phase boundaries to larger packing fractions without altering qualitatively the whole phase behavior.

Since real hematite-silica particles are polydisperse in our simulations we employed polydisperse HEs and HSCs.
...
The average aspect ratio of the simulated HEs and HSCs is equal to $2.9$, i.e.~close to the one of less elongated hematite-silica particles.  
In the following lengths will be expressed in units of $100\nm$. For both systems we simulated $N=1125$ in a orthorhombic box with periodic boundary conditions.
...
Figures~\ref{fig:sim}(a,b) show the EOS of HEs and HSCs. Concerning~\ref{fig:sim}a, it can be seen that, 
similarly to what observed for hematite-silica particles with $\rho=3.72$ and in previous studies~\cite{martchenko2016anisotropic}, 
HEs exhibit two phases throughout all pressures considered in the simulations:  
a para-nematic (pN) phase (for $\phi_0\lesssim 0.45$), where the symmetry axis of the HEs is aligned perpendicularly to the 
external field $\mathbf{B}$, and a nematic phase (N) where HEs self-align also a along a direction perpendicular to $\mathbf{B}$ 
...
We observe that the very same qualitative phase behavior is expected for HEs with larger aspect ratio, according to numerical 
studies reported in Ref.~\cite{martchenko2016anisotropic}.

The phase behavior of HSCs is richer, since they exhibit three phases: a para-nematic (pN) 
...
In this case HSCs allow the emergence of a lamellar phase for $0.45\lesssim\phi\lesssim 0.55$ 
in agreement with experimental results for hematite-silica particles with $\rho=2.82$.
...
is markedly second order as expected. The SmA phase observed in the simulations is purely a field-induced effect in that
it disappears if one switched off the external field (see also SI). 
...
include the form factor and correspond to $\mathbf{q}$-values lying onto the plane identified by the external field (B, i.e.~y-axis) and 
a direction perpendicular to it ($z$-axis), so that a direct comparison with experiments can be made.

The snapshot of HEs is a prototypical example of a nematic phase, whereas no lamellar ordering is present. 
The structure factor similarly reflect the absence of any layering in the system and it can 
be straightforwardly compared with the experimental one show in Fig.~\ref{z_scan}(b). 
On the contrary, in the snapshot of HSCs a clear lamellar ordering is apparent which reflects in the Bragg peaks present in the structure factor.
...
Hematite-silica particles with smaller aspect ratio do exhibit a smectic phase due to their resemblance with HSCs in their midsection,
while particles with larger aspect ratio, being overall more similar to ellipsoids, do not exhibit a lamellar phase.
We note that HSCs do not form a N phase, which is consistent with the phase diagram of monodisperse HSCs
with the same aspect ratio, although hematite-silice particles with $\rho_1=2.82$ undergo a transition to a N phase.
This different phase behavior can be understood by considering the hybrid shape nature of hematite-silica particles
with smaller aspect ratio, which makes them not perfect spherocylinders (see also SI for more information on this).  
