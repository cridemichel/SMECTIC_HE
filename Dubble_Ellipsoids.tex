...
a direction perpendicular to it ($z$-axis), so that a direct comparison with experiments can be made (see SI for more details).
...
\textcolor{red}{A subsequent close inspection of the actual particle images using TEM (Fig.~\ref{z_scan}(a, b)) reveals
  ...
  sections present in our particles with $\rho=2.82$ more closely (see SI). Polydispersity was implemented again as
...
behavior (see also SI for more information on the quantitative image analysis for both particles).}
