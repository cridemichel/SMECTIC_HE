...
and $k$ is parameter by which one can set the strength of the alignment. In our simulation we used $k=10000 k_B T$ since in the real
...
in the simulations.
As insets of Figs.~\ref{fig:sima}(a) and (b) a snapshot of a configuration
...
The distance between Bragg peaks is equal to $2\pi/d$ with $d\approx 1 r.u.$, i.e. about $D$, consistently with the experimental structure
factor shown in Fig.~\ref{z_scan}(c).
